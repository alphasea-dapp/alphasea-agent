...
0.1,BTC"""
    ...
    def test_different_hour_purchased_prediction(self):
        w3 = create_web3()
        contract = create_contract(w3)
        store = create_store(w3, contract)
...
0.1,BTC"""
        ...
        for i in range(1, 24):
            price = predictor._calc_prediction_price(model_id=model_id, execution_start_at=execution_start_at + i * 60 * 60)
            self.assertEqual(price, 100)
